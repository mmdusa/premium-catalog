...
export default function CircularGallery() {
  ...
  return (
    <section className="circ container" aria-label="Circular mini gallery" style={{ ["--n" as any]: n }}>
      ...
      <style jsx>{`
        /* Scoped to .circ so it won't affect hero/background */
        .circ {
          --size: min(70vw, 28rem);
          --radius: calc(var(--size) * 0.46);
        }
        .circ .circ-stage {
          position: relative;
          margin-inline: auto;
        ...
        .circ .circ-card {
          ...
          inset: 0;
          transform-style: preserve-3d;
          transform: rotateY(calc(var(--j) * 1turn)) translateZ(var(--radius));
        }
        .circ figure {
          display: grid;
        ...
        /* Simple hover rotate */
        .circ .circ-card:hover {
          transform: translateZ(var(--radius)) rotateY(calc(var(--j) * 1turn)) scale(1.03);
          transition: transform 0.25s ease;
        }

        /* Make it scrollable horizontally on mobile (no global scroll hacks) */
